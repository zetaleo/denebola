function t_appendGoogleMap(recid, key) {
    var grecid = recid;
    if ("object" == typeof google && "object" == typeof google.maps) t_handleGoogleApiReady(grecid);
    else if (!0 !== window.googleapiiscalled) {
        var runfunc = "window.t_handleGoogleApiReady_" + grecid + ' = function () { t_handleGoogleApiReady("' + grecid + '") }';
        eval(runfunc);
        var addParams = "",
            maplang = "";
        maplang = $("#rec" + recid).find(".t-map").data("map-language"), maplang > "" && 2 == maplang.length && (addParams = "&language=" + maplang);
        var script = document.createElement("script");
        script.type = "text/javascript", script.src = "https://maps.google.com/maps/api/js?key=" + jQuery.trim(key) + "&callback=t_handleGoogleApiReady_" + grecid + addParams, document.body.appendChild(script), window.googleapiiscalled = !0
...
function t_handleGoogleApiReady(recid) {
    $("#rec" + recid).find(".t-map").each((function (index, Element) {
        var el = $(Element),
            arMarkers = window["arMapMarkers" + recid],
            mapstyle = "[]";
        window.isDragMap = !window.isMobile, "" != el.attr("data-map-style") && (mapstyle = eval(el.attr("data-map-style")));
        var myLatLng = arMarkers.length > 0 && new google.maps.LatLng(parseFloat(arMarkers[0].lat), parseFloat(arMarkers[0].lng)),
            myOptions = {
            ...
            i, mrk, marker, markers = [],
            bounds = new google.maps.LatLngBounds;
        for (i in arMarkers) mrk = arMarkers[i], myLatLng = new google.maps.LatLng(parseFloat(mrk.lat), parseFloat(mrk.lng)), marker = new google.maps.Marker({
            position: myLatLng,
            map: map,
            title: mrk.title
        }), bounds.extend(myLatLng), mrk.descr > "" ? attachInfoMessage(marker, mrk.descr) : attachInfoMessage(marker, mrk.title), markers[markers.length] = marker, marker = "";
        ...
        if (arMarkers.length > 1) {
            ...
            var listener = google.maps.event.addListener(map, "idle", (function () {
                (map.getZoom() > parseInt(el.attr("data-map-zoom"), 10) || 0 == map.getZoom()) && map.setZoom(parseInt(el.attr("data-map-zoom")), 10), map.getZoom() > 16 && map.setZoom(16), google.maps.event.removeListener(listener)
            }))
        }
...
function t_appendYandexMap(recid, key) {
    ...
    if ("object" == typeof ymaps && "function" == typeof ymaps.Map) t_handleYandexApiReady(recid);
    else if (!0 !== window.yandexmapsapiiscalled) {
        ...
        var script = document.createElement("script");
        script.type = "text/javascript", script.src = "https://api-maps.yandex.ru/2.1/?lang=" + maplang + "&coordorder=latlong&onload=t_handleYandexApiReady_" + yarecid, key > "" && (script.src = script.src + "&apikey=" + key), document.body.appendChild(script), window.yandexmapsapiiscalled = !0
    } else setTimeout((function () {
...
function t_handleYandexApiReady(recid) {
    $("#rec" + recid).find(".t-map").each((function (index, Element) {
        ...
        var myLatlng = arMarkers.length > 0 && [parseFloat(arMarkers[0].lat), parseFloat(arMarkers[0].lng)],
            ...
            map = new ymaps.Map(Element, myStates),
            i, mrk, myGroup = new ymaps.GeoObjectCollection({}),
        ...
        for (i in Object.keys(mobilePanelStyles).forEach((function (name) {
            ...
            }))), arMarkers) mrk = arMarkers[i], myLatlng = [parseFloat(mrk.lat), parseFloat(mrk.lng)], myGroup.add(new ymaps.Placemark(myLatlng, {
        ...
        })) : zoom > 0 && (0 == map.getZoom() || map.getZoom() > zoom) && map.setZoom(zoom), map.events.add("sizechange", (function () {
