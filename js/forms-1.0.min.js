        ...
        mask: function (mask, settings) {
            ...
            return this.trigger("unmask").each(function () {
                ...
                input.one("unmask", function () {
                    ...
                    if (input.data('tilda-mask').indexOf('+7') === 0) {
                        var value = input.val().replace(/\D/g, '');
                        if (value.indexOf('78') === 0) {
                            input.val(value.substring(2))
                        }
...
function t_form_dict(msg) {
    ...
    $(document).ready(function () {
                ...
                if (req == 1 && ((val.length == 0 && valnospace.length == 0) || (($(this).attr('type') == 'checkbox' || $(this).attr('type') == 'radio') && $(this).closest('form').find('[name="' + $(this).attr('name') + '"]:checked').length == 0))) {
                    ...
                    switch (rule) {
                        ...
                        case 'date':
                            ...
                            if (valnospace.length > 0 && !valnospace.match(format[$(this).data('tilda-dateformat')] || /^[0-9]{1,4}[\-\.\/][0-9]{1,2}[\-\.\/][0-9]{1,4}$/gi)) {
            ...
            for (var i = 0; i < arErrors.length; i++) {
                ...
                for (j = 0; j < arErrors[i].type.length; j++) {
                    ...
                    if (isShowCommonErrors == 1) {
                        ...
                        if ($errItem.length > 0) {
                            if ($errItem.text() == '' && t_form_dict(error) > '') {
                                if (sCommonError.indexOf(t_form_dict(error)) == -1) {
                                    sCommonError = sCommonError + '<p class="t-form__errorbox-item">' + t_form_dict(error) + '</p>'
                                }
                            } else {
                    ...
                    if (isFieldErrorBoxExist) {
                        if (sError == '') {
                            if (t_form_dict(error + 'field') > '') {
                                sError = t_form_dict(error + 'field')
                            } else {
                                if (t_form_dict(error) > '') {
                                    sError = t_form_dict(error)
                                }
                            }
                        }
                        if (sError > '') {
        ...
        window.tildaForm.showErrors = function ($jform, arErrors) {
            ...
            for (var i = 0; i < arErrors.length; i++) {
                ...
                if (i == 0 && arErrors[i].obj == 'none') {
                    ...
                    $errItem.html(t_form_dict('emptyfill'));
                ...
                for (j = 0; j < arErrors[i].type.length; j++) {
                    ...
                    if (isShowCommonErrors == 1) {
                        ...
                        if (($errItem.attr('data-rule-filled'))) {
                            ...
                            if ($errItem.text() == '' && t_form_dict(error) > '') {
                                $errItem.html(t_form_dict(error))
                            } else {
                                sError = $errItem.eq(0).text()
                            }
                            $errItem.css('display', 'block').show()
        ...
        window.tildaForm.payment = function ($jform, arNext) {
            ...
            if ($jform.find('.js-successbox').length > 0) {
                ...
                if (t_form_dict('successorder')) {
                    $jform.find('.js-successbox').html(t_form_dict('successorder'))
                }
            ...
            if (arNext.type == 'link') {
                ...
                if (arNext.type == 'form') {
                    ...
                    $('body').append(html);
                    tildaForm.clearTCart($jform);
                    if ($('#js-tilda-payment-formid').attr('action') > '') {
        ...
        window.tildaForm.sendStatAndShowMessage = function ($jform, arArgs, sendStat) {
            ...
            var successcallback = $jform.data('success-callback');
            if (successcallback && successcallback.length > 0 && successcallback === 't396_onSuccess' && typeof window.t396_onSuccess === "function") {
                eval(successcallback + '($jform)')
            } else if (successcallback && successcallback.length > 0 && successcallback !== 't396_onSuccess') {
                eval(successcallback + '($jform)')
            }
                    ...
                    success: function (json) {
                        ...
                        if (json && json.error) {
                            ...
                            if (window.Tilda && typeof Tilda.sendEventToStatistics == 'function') {
                                ...
                                try {
                                    if (typeof ga != 'undefined' && ga && window.mainTracker != 'tilda') {
                                        ga('send', {
                                            'hitType': 'pageview',
                                            'page': virtPage,
                                            'title': virtTitle
                        ...
                        if (!error || (error.status == 0 && ts_delta < 100)) {
                            ...
                            if (error && (error.status >= 500 || error.status == 408 || error.status == 410 || error.status == 429 || error.statusText == 'timeout') && window.tildaForm.endpoint.indexOf('forms.tilda') !== -1) {
                                ...
                                if (error && error.responseText > '') {
                                    $allError.html('Please, try again later.')
                                } else {
                                    if (error && error.statusText) {
                                        $allError.html('Ваше сообщение отправлено.')
                                    } else {
